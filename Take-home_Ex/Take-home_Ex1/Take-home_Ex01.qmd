...
top_age_band <- tail(levels(national_data$AgeBand), 1)  # "90+"
...
pyramid_plot <- ggplot(national_data, aes(x = AgeBand, y = Total, fill = Sex)) +
  ...
  labs(
    title = "Population Pyramid (5-Year Age Bands)",
    x = "Age Group",
    y = "Population"
  ) +
  ...
  annotate("text", x = top_age_band, y = -max_pop * 0.9, label = "Males", color = "#7A90C9", fontface = "bold", size = 5) +
  annotate("text", x = top_age_band, y = max_pop * 0.9, label = "Females", color = "#F7A1A0", fontface = "bold", size = 5)
...
stacked_bar_plot <- ggplot(stacked_data, aes(x = AgeBand, y = Proportion, fill = Sex)) +
  ...
  labs(
    title = "Age-Sex Distribution (5-Year Age Bands)",
    x = "Age Group",
